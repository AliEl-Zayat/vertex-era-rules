import { addTypeAnnotation, analyzeReactImport, detectIconEnvironment, getTypeAnnotationStrategy, selectMemoStrategy, updateImports, validateExportDefaultDeclaration, validateJSXElement, } from '../../utils/ast-helpers.js';
...
const iconRules = {
    'single-svg-per-file': {
        meta: {
            type: 'problem',
            docs: {
                description: 'Ensure only one SVG icon per file. This improves code organization, makes icons easier to find and maintain, and enables better tree-shaking in production builds.',
            ...
            schema: [],
            messages: {
                multipleSvgs: 'Multiple SVG icons found ({{count}} total). Each icon must be in its own file for better maintainability and tree-shaking. Consider splitting into separate icon files.',
            },
        },
        create(context) {
            let svgCount = 0;
    ...
    'svg-currentcolor': {
        meta: {
            type: 'problem',
            docs: {
                description: 'Ensure single-color SVGs use currentColor. This allows icons to inherit the text color from their parent element, making them more flexible and easier to theme.',
                category: 'Best Practices',
                recommended: 'error',
            ...
            schema: [],
            messages: {
                useCurrentColor: 'Single-color SVG should use "currentColor" instead of {{attribute}}="{{value}}". This allows the icon to inherit the text color from its parent, making it more flexible and themeable.',
            },
        },
        create(context) {
            return {
                                ...
                                colorIssues.forEach((issue) => {
                                    context.report({
                                        ...
                                        fix(fixer) {
                                            if (!issue.node.value)
                                                return null;
                                            return fixer.replaceText(issue.node.value, '"currentColor"');
                                        },
                                    });
                                });
                            }
    ...
    'memoized-export': {
        meta: {
            type: 'problem',
            docs: {
                description: 'Ensure icon components are memoized using React.memo. This prevents unnecessary re-renders when the icon is used in parent components, improving performance especially when icons are used frequently.',
                category: 'Best Practices',
                recommended: 'error',
            ...
            schema: [],
            messages: {
                memoizeExport: 'Icon component "{{componentName}}" should be wrapped with React.memo to prevent unnecessary re-renders. Use: export default memo({{componentName}});',
            },
        },
        create(context) {
            let exportNode = null;
            ...
            function checkExport(ctx, node, svg) {
                try {
                    // Validate node structure
                    ...
                    const isMemoized = node.declaration.type === 'CallExpression' &&
                        ...
                        if (componentName) {
                            ctx.report({
                                ...
                                fix(fixer) {
                                    ...
                                    const typeStrategy = getTypeAnnotationStrategy(envDetection.environment, importAnalysis.style);
                                    // Step 5: Generate all fixes
                                    const fixes = [];
                                    // Step 5a: Update imports (add memo if needed)
                                    const importUpdate = updateImports(memoStrategy, importAnalysis.importNode, programNode);
                                    ...
                                    if (node.declaration.type === 'FunctionDeclaration') {
                                        ...
                                        if (functionNode.params.length > 0) {
                                            ...
                                            if (firstParam.type === 'Identifier' &&
                                                ...
                                                functionText = functionText.replace(paramPattern, `(${paramName}: ${typeStrategy.propsType})`);
                                            }
                                        }
                                        else {
                                            ...
                                            functionText = functionText.replace(emptyParamsPattern, `(props: ${typeStrategy.propsType})`);
                                        }
                                        // Replace the entire export statement with function + export
                                        fixes.push(fixer.replaceText(node, `${functionText}\n\nexport default ${memoStrategy.memoReference}(${componentName});`));
                                    }
                                    else {
                                        ...
                                        if (componentNode) {
                                            const typeAnnotationResult = addTypeAnnotation(componentNode, typeStrategy, programNode);
                                        ...
                                        fixes.push(fixer.replaceText(node, `export default ${memoStrategy.memoReference}(${componentName});`));
                                    }
                                    return fixes;
                                },
                            });
...
// Helper functions
function checkForMultipleColors(node, colors) {
...
function findColorAttributeIssues(node) {
    const issues = [];
...
function getComponentNode(declaration) {
...
//# sourceMappingURL=eslint-plugin-icon-rules.js.map